...
contract Victim {
    ...
    function setPlayers(address _address) {
        players[0] = _address;
    }
    // modifier onlyPartner(){
    //     //省略一些控制语句
    ...
    function setAddressWithModifier(address _addr)   public {

         //erc20 = Victim(this).returnERC20(_addr);//this.call(bytes4(keccak256("returnERC20(address)")), _addr);//ERC20(_addr);//ERC20(players[0]);
        // addr = msg.sender;
        //addr = msg.sender >> 2;
        erc20 = ERC20(_addr);//returnERC20(_addr);
    }
    
    function returnERC20(address _addr) private returns(ERC20) {
        return ERC20(_addr);
    }
