...
4:判断这些后续节点是不是internal call
5:找到含有internall call的节点后，就进入到called Function 其实就是cfg跳入到call graph
6:遍历calledFunction中的所有节点，为了找到所有含有high_level_call或low_level_call的节点,如果找到external call
  如果找到就进行脏数据的分析（待解决），如果分析结果为干净，则继续找含有interanl call的节点，返回第五步
"""
